...
Page({
  ...
  data: {
    ...
    //老师代码开始
    propertyChildIds: '',
    sizeData: [] //点击规格后请求的接口的返回值  订单页面能用
  },
    ...
    wx.request({
      ...
      success: function(res) {
        ...
        _this.setData({
          ...
          goodsPrice: res.data.data.basicInfo.minPrice
        })
  ...
  //这是自己的方式去做购物车  有点麻烦 还是老师的好
  size1: function(e) {
    this.setData({
      opID1: e.currentTarget.dataset.propertyid,
      valID1: e.currentTarget.dataset.id
    })
    console.log(this.data.opID1, this.data.opID2, this.data.opID3)

    if (this.data.goodsDetail.properties.length == 2 && this.data.opID2 == '') {
      return false
    } else if (this.data.goodsDetail.properties.length == 3) {
      if (this.data.opID2 == '' || this.data.opID3 == '') {
        console.log("222")
        return false
      } else {
        var str = this.data.opID1 + ':' + this.data.valID1 + ',' + this.data.opID2 + ':' + this.data.valID2 + ',' + this.data.opID3 + ':' + this.data.valID3
  ...
  size2: function(e) {
    this.setData({
      opID2: e.currentTarget.dataset.propertyid,
      valID2: e.currentTarget.dataset.id
    })
    console.log(this.data.opID1, this.data.opID2, this.data.opID3)

    if (this.data.goodsDetail.properties.length == 2 && this.data.opID1 == '') {
      return false
    } else if (this.data.goodsDetail.properties.length == 3) {
      if (this.data.opID1 == '' || this.data.opID3 == '') {
        return false
      } else {
        var str = this.data.opID1 + ':' + this.data.valID1 + ',' + this.data.opID2 + ':' + this.data.valID2 + ',' + this.data.opID3 + ':' + this.data.valID3
        this.requestPrice(str)
  ...
  size3: function(e) {
    this.setData({
      opID3: e.currentTarget.dataset.propertyid,
      valID3: e.currentTarget.dataset.id
    })
    console.log(this.data.opID1, this.data.opID2, this.data.opID3)
    if (this.data.goodsDetail.properties.length == 3 && this.data.opID1 == '' || this.data.opID2 == '') {
      return false
    } else {
      var str = this.data.opID1 + ':' + this.data.valID1 + ',' + this.data.opID2 + ':' + this.data.valID2 + ',' + this.data.opID3 + ':' + this.data.valID3
      this.requestPrice(str)
  ...
  addCartTrue: function() {
    let _this = this;
    let arr = []
    if (_this.data.opID1 == _this.data.opID2 == _this.data.opID3 == "") {
      arr = []
      console.log("65465")
    }
    if (_this.data.opID1 != "" && _this.data.opID2 == "" && _this.data.opID3 == '') {
      arr = [{
        optionId: _this.data.opID1,
        optionValueId: _this.data.valID1
      }]
    }
    if (_this.data.opID2 != "" && _this.data.opID1 == "" && _this.data.opID3 == '') {
      arr = [{
        optionId: _this.data.opID2,
        optionValueId: _this.data.valID2
      }]
    }
    if (_this.data.opID3 != "" && _this.data.opID1 == "" && _this.data.opID2 == '') {
      arr = [{
        optionId: _this.data.opID3,
        otionValueId: _this.data.valID3
      }]
    }
    if (_this.data.opID1 != "" && _this.data.opID2 != "" && _this.data.opID3 == '') {
      arr = [{
    ...
    if (_this.data.opID1 != "" && _this.data.opID3 != "" && _this.data.opID2 == '') {
      arr = [{
        optionId: _this.data.opID1,
        optionValueId: _this.data.valID1
      }, {
        optionId: _this.data.opID3,
        optionValueId: _this.data.valID3
      }]
    }
    if (_this.data.opID2 != "" && _this.data.opID3 != "" && _this.data.opID1 == '') {
      arr = [{
        optionId: _this.data.opID2,
        optionValueId: _this.data.valID2
      }, {
  ...
  goOrderConfirm: function() {
    ...
    //判断规格是否选中
    if (_this.data.goodsDetail.properties) {
      if (_this.data.goodsDetail.properties.length == 1 && _this.data.opID1 == '') {
        wx.showToast({
          title: '请选择规格',
        })
      } else if (_this.data.goodsDetail.properties.length == 2 && _this.data.opID1 == '') {
        wx.showToast({
          title: '请选择规格',
        })
      } else if (_this.data.goodsDetail.properties.length == 2 && _this.data.opID2 == '') {
        wx.showToast({
          title: '请选择规格',
        })
      } else if (_this.data.goodsDetail.properties.length == 3 && _this.data.opID1 == '') {
        wx.showToast({
          title: '请选择规格',
        })
      } else if (_this.data.goodsDetail.properties.length == 3 && _this.data.opID2 == '') {
        wx.showToast({
          title: '请选择规格',
        })
      } else if (_this.data.goodsDetail.properties.length == 3 && _this.data.opID3 == '') {
        wx.showToast({
          title: '请选择规格',
        })
      } else {
        // if (_this.data.goodsDetail.properties.length == 1) {
        //   var str = this.data.opID1 + ':' + this.data.valID1
        // }

        // if (_this.data.goodsDetail.properties.length == 2) {
        //   var str = this.data.opID1 + ':' + this.data.valID1 + ',' + this.data.opID2 + ':' + this.data.valID2
        // }
        // if (_this.data.goodsDetail.properties.length == 3) {
        //   var str = this.data.opID1 + ':' + this.data.valID1 + ',' + this.data.opID2 + ':' + this.data.valID2 + ',' + this.data.opID3 + ':' + this.data.valID3
        // }
        //跳转到确定订单页  并传递当前商品信息  和选中的规格信息

        var type = "详情页"
        wx.navigateTo({
          url: '/packageB/pages/orderConfirm/orderConfirm?goodsDetail=' + JSON.stringify(_this.data.goodsDetail) + '&sizeData=' + JSON.stringify(_this.data.sizeData) + '&number=' + _this.data.number + '&goodsPrice=' + _this.data.goodsPrice + '&type=' + type,
          success: function(res) {
            _this.setData({
              goodsAttrFlag: false
            })
      ...
      var type = '详情页'
      wx.navigateTo({
        url: '/packageB/pages/orderConfirm/orderConfirm?goodsDetail=' + JSON.stringify(_this.data.goodsDetail) + '&number=' + _this.data.number + '&goodsPrice=' + _this.data.goodsPrice + '&type=' + type,
        success: function(res) {
          _this.setData({
            goodsAttrFlag: false
          })
