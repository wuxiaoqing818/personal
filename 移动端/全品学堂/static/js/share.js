...
export const commonShare = (_this, shareTitle, shareUrl, shareImg, shareDesc) => {
    ...
    //一年级数据
    axios.get("../../../static/data/home/firstGradeNav.json")
        .then(res => {
            if (res.status == 1) {
                ...
                wx.config({
                    ...
                    appId: data.id, // 必填，公众号的唯一标识

                    timestamp: data.id, // 必填，生成签名的时间戳

                    nonceStr: data.id, // 必填，生成签名的随机串

                    signature: data.id, // 必填，签名，见附录1
