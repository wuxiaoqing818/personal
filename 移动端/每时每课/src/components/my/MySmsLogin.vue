    ...
    <div class="wxq-content">
      ...
      <div class="wxq-write">
        <p :class="flag1">
          ...
          <span @click="getMessage()" v-show="time">获取验证码</span>
          <span v-show="!time">获取验证码({{count}})</span>
        </p>
        <p :class="flag2">
          <input
            type="text"
        ...
        <div>
          <span>请输入短信验证码</span>
          <span @click="passLogin()">使用密码登录</span>
        </div>
  ...
  methods: {
    ...
    getMessage() {
      ...
      if (!reg.test(this.phone)) {
        ...
        //符合正则调用定时器
        this.$options.methods.getCode.bind(this)();
    ...
    login() {
      ...
      } else if (!reg2.test(this.message)) {
        this.$toast({
          message: "验证码 必须是 六个字符"
        });
      } else {
        http(
          "post",
          ...
          if (res.data.code == 200) {
            ...
            if (res.data.data.is_new == 1) {
              ...
              sessionStorage.setItem("userMessage", this.message);
              // sessionStorage.setItem("token", res.data.data.remember_token);
              this.$store.commit("setUserInfo", res.data.data);
              this.$router.push("/my-set-pass");
            } else {
              sessionStorage.setItem("userPhone", this.phone);
              sessionStorage.setItem("userMessage", this.message);
              this.$store.commit("setUserInfo", res.data.data);
